 ...
 * know the seed.<br>
 * <br>
 * This is the original implementation by Bob Jenkins with some minor
 * changes.<br>
 * <br>
 * <b>Changelog:</b><br>
 * <ul>
...
public final class ISAAC {
    ...
    public static final int SIZEL = 8;					// Log of size of rsl[] and mem[]
    public static final int SIZE = 1 << SIZEL;		// Size of rsl[] and mem[]
    public static final int MASK = (SIZE - 1) << 2;	// For pseudorandom lookup
    public int count;						// Count through the results in rsl[]
    public final int[] rsl;						// The results given to the user
    public final int[] mem;						// The internal state
    public int accumulator;							// Accumulator
    public int lastResult;							// The last result
    public int counter;							// Counter, guarantees cycle is at least 2^40
    ...
    public void generate() {
        ...
        for (i = 0; i < SIZE; ++i) {
            ...
            accumulator += mem[(i + SIZE / 2) & (SIZE - 1)];
            mem[i] = y = mem[((x) & MASK) >> 2] + accumulator + lastResult;
            rsl[i] = lastResult = mem[((y >> SIZEL) & MASK) >> 2] + x;
        }
    }
     ...
     * @return the next int in the series
     */
    public int next() { 
        if (0 == count--) {
            generate();
